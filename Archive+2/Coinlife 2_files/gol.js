...
var F = {
    ...
    aimg: function(o) {
        ...
        f.innerHTML = '<form method="post" enctype="multipart/form-data" action="' + u + '" target="' + n + '_i" ><input type="file" onchange="this.form.submit();F.upimg(\'' + n + '_i\',\'' + fun + '\',' + sf + ');" name="' + n + '" style="margin-left:-140px;cursor:pointer;" /></form><iframe name="' + n + '_i" id="' + n + '_i" style="display:none"></iframe>';
        o.ob = f;
...
var A = {
    ...
    sel: function(a, url, type) {
        var b = a.nextSibling;
        var d = [];
        while (b && b.tagName.toLowerCase() == 'select') {
            d.push(b);
            b = b.nextSibling
        };
        ...
        this.aj(url, (type || a.name) + '=' + a.value, function(da) {
            if (!da) return;
            var se = A.$$('select');
            se.className = a.className || 'select';
            se.name = da.name;
            if (da.data) {
                se.options.add(new Option('', ''));
                for (var i = 0; i < da.data.length; i++) {
                    se.options.add(new Option(da.data[i].val, da.data[i].key))
                };
                a.parentNode.appendChild(se);
                if (da.fun) {
...
(function(w) {
    ...
    function div(ob) {
        this.cj = function() {
            ...
            var _w = Math.max(d.documentElement.scrollWidth, d.documentElement.clientWidth);
            var _h = Math.max(d.documentElement.scrollHeight, d.documentElement.clientHeight);
            with(this.G_d.style) {
                position = 'absolute';
                zIndex = 99;
                top = '0px';
            ...
            if (typeof(ob.css) == 'string') {
                ...
                if (ob.css == 'ui_confirm') {
                    ...
                    if (typeof(ob.obj) == 'object') {
                        ...
                        bd.innerHTML = '<img src="/public/images/ui_load.gif" alt="加载中" style="margin:10px 30px;" />';
